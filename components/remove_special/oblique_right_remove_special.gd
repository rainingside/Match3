...
class_name ObliqueRightRemoveSpecial
...
func p_remove_special(special_location:RemoveSpecialLocationData, data:GameData) -> RemoveSpecialData:
	...
	if target_block == null or target_block.Data.p_get_block_special_type() != Enums.BlockSpecialTypes.ObliqueRight:
		return remove_data
	...
	while tx < data.Row and ty < data.Column:
